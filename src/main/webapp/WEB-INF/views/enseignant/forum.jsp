...
<div class="topbar">
  ...
  <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalPost">+ Nouveau Post</button>
</div>
  ...
  <c:forEach var="post" items="${posts}">
    <div class="card mb-3">
      <div class="card-header d-flex justify-content-between">
        ...
        <small class="text-muted">${post.auteur.prenom} ${post.auteur.nom} - ${post.creeLe}</small>
      </div>
      <div class="card-body">
        <p>${post.contenu}</p>
        <c:forEach var="reply" items="${post.replies}">
          <div class="ms-4 p-2 bg-light rounded mb-2">
            <small class="fw-bold text-muted">${reply.auteur.prenom} ${reply.auteur.nom} :</small>
            <p class="mb-0 small">${reply.contenu}</p>
          </div>
        </c:forEach>
        <form method="post" action="/enseignant/forum/${post.id}/repondre" class="mt-2 d-flex gap-2">
          <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
          <input type="text" name="contenu" class="form-control form-control-sm" placeholder="Votre reponse..." required>
          <button type="submit" class="btn btn-outline-primary btn-sm"><i class="bi bi-send"></i></button>
        </form>
      </div>
    </div>
  </c:forEach>
  <a href="/enseignant/cours/${course.id}/chapitres" class="btn btn-secondary"><i class="bi bi-arrow-left me-1"></i>Retour</a>
</div>
<div class="modal fade" id="modalPost" tabindex="-1">
  <div class="modal-dialog"><div class="modal-content">
    ...
    <form method="post" action="/enseignant/cours/${course.id}/forum">
      <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
